# Version 1.0.10 bundled dependencies

The portable package is assembled without running either upstream installer.
The 1.0.10 branding, documentation, and release-automation update reuses the
exact pinned and verified backend binary shipped by 1.0.6 and the
controller-side Stripe/PayPal behavior shipped by 1.0.8. No dependency or
payment implementation changed. No Stripe or PayPal merchant SDK/library is
added; the existing Myst node and Mysterium Pilvytis service remain the order
and wallet-credit authority.
